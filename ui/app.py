...
with st.sidebar:
    ...
    if st.session_state.pop("_navigate_to_results", False):
    ...
    st.radio(
        ...
        key="nav_page",
        disabled=not st.session_state.results_unlocked,
        help="Enabled after you click Analyze once.",
    )
...
if current_page == "Input":
    ...
    has_csv = uploaded_file is not None or st.session_state.get("uploaded_file_bytes")
    ...
    analyze = st.button(
        "Analyze Finances and Generate Detailed AI Report",
        use_container_width=True,
        disabled=not has_csv,
    )

    if analyze and has_csv:
        print("Analyze button clicked", flush=True)
        if uploaded_file is not None:
            st.session_state.uploaded_file_bytes = uploaded_file.getvalue()
            st.session_state.uploaded_file_name = uploaded_file.name
        st.session_state.results_unlocked = True
        st.session_state._navigate_to_results = True
...
if current_page == "Results":
    ...
    if st.session_state.run_analysis:
        ...
        csv_bytes = st.session_state.get("uploaded_file_bytes")
        if not csv_bytes:
            st.error("No transaction CSV is available. Go back to Input and upload a file.")
            st.stop()

        st.markdown("##### Pipeline")
        pipeline_ph = st.empty()
        show_pipeline(pipeline_ph, completed=0, active=0)
        ...
        if not completed_result:
            st.info("No analysis results yet. Run Analyze from the Input page.")
        else:
            st.markdown("##### Pipeline")
            ...
            render_recommendations(
                completed_result["recommendations"],
                completed_result.get("recommendation_generation_seconds"),
            )
            render_report(
                completed_result["report"],
                completed_result["report_path"],
                completed_result["report_generation_seconds"],
            )
